...
- [x] GIF アニメーションエクスポート — Issue #14
  - `gif` crate 0.13, ExportFormat enum (PNG/GIF), FPS スライダー (1-30)
  - 全時間ステップ連番レンダリング → GIF エンコード (無限ループ)
  - render_frame() ヘルパーで PNG/GIF コード共有
- [x] Publication-quality エクスポート — Issue #15
  - bitmap_font.rs: 5x7 ビットマップフォント (ASCII)
  - 軸ラベル (Longitude/Latitude) + ティックマーク (0°-360° / -90°-90°)
  - 縦カラーバー (min/mid/max ラベル付き) + タイトル + データ枠線
- [x] 波数フィルタスライダー — Issue #16
  - ispack-rs 連携: Sphere::gaussian → spectral transform → truncation → inverse
  - spectral_filter.rs: wavenumber_filter(), detect_n_trunc(), 緯度順序検出
  - Inspector "Spectral Filter" セクション + `W` キートグル

### Done (v0.4 P2 — スクリプト双方向)

- [x] Code → GUI 反映 — Issue #10
  - codegen/parser.rs: ParsedSettings + parse_python() (regex-free パターンマッチ)
  - 対応: 変数名, time/level, colormap, vmin/vmax, 投影法, オーバーレイ
  - CodePanel: 編集可能 TextEdit + Run/Reset ボタン + ステータス表示
- [x] レシピ保存・読み込み (.py) — Issue #11
  - Save ボタン: Python コードを .py ファイルとして保存 (GeoScope ヘッダー付き)
  - Load ボタン: .py 読み込み → Run で GUI に適用
- [x] Rhai スクリプトエンジン統合 — Issue #12
  - rhai crate 1.x, codegen/rhai_engine.rs
  - API: set_variable, set_time, set_level, set_colormap, set_view, set_range 等
  - ScriptSettings → UiState 適用, 11 ユニットテスト
- [x] 外部 JSON/CSV アノテーション — Issue #13
  - data/trajectory_loader.rs: JSON (object/array) + CSV パーサー
  - Inspector "Load File..." ボタン (rfd ダイアログ)
  - SessionState: trajectory 設定の永続化修正

### Done (v0.5 — LLM Copilot)

目標: 「AI と対話しながら探索する」

- [x] チャットサイドバー (Explain + Explore) — Issue #17
  - copilot/llm_client.rs: Claude API クライアント (ureq, バックグラウンドスレッド)
  - copilot/context.rs: GFD ドメイン知識 + 現在ビュー状態のコンテキスト生成
  - 右パネル 3 タブ目 "Copilot": チャット UI + Explain/Explore クイックアクション
  - ANTHROPIC_API_KEY 環境変数で認証
- [x] コマンドパレット (Cmd+K) — Issue #18
  - codegen/cmd_palette.rs: ローカルパターンマッチ + fuzzy search
  - Cmd+K でフローティングオーバーレイ表示
  - 対応: ビュー切替, カラーマップ, 投影法, オーバーレイ, 変数選択, エクスポート
  - Enter で即実行、クリックで選択

### Done (v0.5+ — ドキュメント)

- [x] README.md を v0.5 実装に合わせて更新
  - ステータスバッジ v0.4 → v0.5
  - AI & Automation セクション新設 (LLM Copilot, Command Palette, Recipe Save/Load)
  - Interaction セクション拡充 (Interactive Profile, GIF Export, Rhai Scripting, Spectral Filter 等)
  - キーボードショートカット追加 (W, [/], Cmd+K)
  - Tech Stack に ispack-rs, rhai, ureq, gif 追加
  - Architecture ツリーに copilot/, codegen/ 全ファイル, data/ 新ファイル追加
  - Roadmap を v0.3〜v0.5 の実装内容に合わせて更新

### Done (v0.6 WIP — i18n, MCP, スペクトル改善)

- [x] E(m) スペクトル表示改善
  - E(m): log-log 折れ線 → 線形横軸 + log 縦軸の棒グラフに変更
  - 離散波数構造が一目で分かるように (RH 波 R=4 で m=4,8,12,... のみに棒)
  - 参照傾き (n⁻³, n⁻⁵ᐟ³) は E(n) のみに表示 (E(m) では非表示)
  - E(n)/E(m) 両方: データ点がある波数値に直接ラベル表示 (自動間引き付き)
- [x] i18n (日英切替, src/i18n.rs)
- [x] MCP listener (src/mcp/)
- [x] Copilot context 拡張
- [x] Copilot チャット markdown レンダリング
  - render_markdown(): 見出し, 太字, 箇条書き, コードブロック, インラインコード対応
  - render_inline_markdown(): `**bold**` と `` `code` `` のインライン解析
  - ダークテーマ配色 (コード: 暗背景+黄緑, 見出し: 白, 箇条書き: ティール)

### Done (v0.6+ — 時間スペクトル + フィルタ)

- [x] 時間周波数スペクトル E(ω)
  - temporal_filter.rs: ispack-rs RealFftPlan で時系列 → パワースペクトル計算
  - Spectrum ビューに E(ω) タブ追加 (log-log, amber カラー, 参照傾きなし)
  - profile_point の時系列データを FFT 分析
  - 7 ユニットテスト (正弦波ピーク検出, 定数入力, Parseval 定理, フィルタ恒等性・ローパス)
- [x] 時間ローパスフィルタ
  - temporal_lowpass_filter(): 周波数領域でカットオフ以上をゼロ化 → 逆変換
  - Inspector Spectral Filter セクションに Temporal Filter UI (checkbox + cutoff slider)
  - Profile TimeSeries ビューにフィルタ適用 (高周波除去で平滑化)
- [x] i18n: temporal_filter, click_for_spectrum (EN/JA)
...
└── src/
    ...
    │   ├── spectral_filter.rs # 波数フィルタ (ispack-rs 連携, spectral truncation)
    │   ├── temporal_filter.rs # 時間FFTスペクトル + ローパスフィルタ (ispack-rs RealFftPlan)
    │   └── trajectory_loader.rs # 外部軌跡読み込み (JSON/CSV)
    ├── renderer/
    ...
    │   ├── spectrum.rs       # E(n)/E(m)/E(ω) スペクトル (egui painter, log-log/bar)
    ...
    │   ├── bitmap_font.rs    # 5x7 ビットマップフォント (Publication export 用)
    │   └── export.rs         # PNG/GIF エクスポート (image + gif crate)
    ├── copilot/
    │   ├── mod.rs            # LLM Copilot ハブ
    │   ├── llm_client.rs     # Claude API クライアント (ureq, async via thread)
    │   └── context.rs        # GFD ドメイン知識 + ビューコンテキスト生成
    ├── codegen/
    │   ├── mod.rs            # コード生成ハブ
    │   ├── python.rs         # Python (xarray+cartopy+matplotlib) 生成
    │   ├── parser.rs         # Python → GUI 逆パーサー (Code→GUI 双方向同期)
    │   ├── rhai_engine.rs    # Rhai スクリプトエンジン (GeoScope API)
    │   └── cmd_palette.rs    # コマンドパレット (fuzzy match + アクション実行)
...
| PNG/GIF | image + gif | 0.25 / 0.13 |
| Spectral | ispack-rs | path dep |
| Script | rhai | 1 |
| JSON | serde_json | 1 |
| HTTP | ureq | 2 |
